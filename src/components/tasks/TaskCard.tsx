    ...
    return (
        <div
            ...
            className={`relative rounded-md border border-slate-700 bg-slate-800 p-3 shadow-sm hover:bg-slate-700/50 transition-all duration-200 cursor-pointer group ${
                isDragging ? 'opacity-50 scale-95 bg-slate-700/30 border-dashed border-slate-500' : 'hover:shadow-lg hover:shadow-slate-900/20'
            }`}
        ...
        >
            ...
            >
                <div className="flex flex-col gap-0.5 transform transition-transform duration-200 hover:scale-110">
                    <div className="flex gap-0.5">
                        <div className="w-0.5 h-0.5 bg-white rounded-full transition-all duration-200 hover:bg-cyan-300"></div>
                        <div className="w-0.5 h-0.5 bg-white rounded-full transition-all duration-200 hover:bg-cyan-300"></div>
                    </div>
                    <div className="flex gap-0.5">
                        <div className="w-0.5 h-0.5 bg-white rounded-full transition-all duration-200 hover:bg-cyan-300"></div>
            ...
            <div className="select-none pl-2 transition-all duration-200">
                <h4 className="text-sm font-medium text-slate-200 transition-colors duration-200 group-hover:text-white">{task.title}</h4>
                <div className="mt-2 flex items-center justify-between">
                    <span className="text-xs text-slate-400 transition-colors duration-200 group-hover:text-slate-300">{task.priority}</span>
                    {/* placeholder para el avatar del asignado */}
                    <div className="h-6 w-6 rounded-full bg-slate-600 transition-all duration-200 group-hover:bg-slate-500 group-hover:scale-105"></div>
                </div>
            </div>
        </div>
    );
