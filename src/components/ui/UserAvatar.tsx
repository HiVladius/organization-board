  ...
  const avatarElement = user.avatar
    ? (
      <img
        ...
        className={`${
          sizeClasses[size]
        } rounded-full object-cover border-2 border-cyan-500 ${className}`}
      />
    )
    : (
      <div
        className={`${
          sizeClasses[size]
        } rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold ${className}`}
      >
